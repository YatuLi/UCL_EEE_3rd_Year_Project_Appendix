...
abstract public class NetworkInterface implements ModuleCommunicationListener {
	 ...
	 * 
	 * @since 25/01/2017
	 * <b>This setting is different from  {@link transmitRange} and {@link transmitSpedd}<b>
	 */
	...
	// ****************A new method written by Yatu 
	public boolean isBelowThreshold(double thresholdPercent){
		boolean belowThreshold = false; // As a local variable, it has been initialised before being used in the following stages.
		System.out.println("Initial Energy from NI: " + host.getComBus());
		/* A test shows the initial energy is not stored on the ComBus, so we would be better to access it from the EnergyModel.java
		 * Since some of the parameters are not necessarily to be logged on the ComBus at all, the initial energy is not like the current energy level.
		 * 
		 * Needed to be played with after a while.
		 */
		
		if (host.getComBus().getDouble(EnergyModel.ENERGY_VALUE_ID, 0) <= thresholdPercent * 1){ 
			belowThreshold = true;
		}
	...
	public boolean isScanning() {
		...
		// a New method created by Yatu for an Energy Threshold
//		if (!isBelowThreshold(thresholdPercent)){
//			return false;
//		}
	...
	// All of new methods devised by Yatu
