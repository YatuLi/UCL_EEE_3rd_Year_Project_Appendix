...
public class EnergyModel implements ModuleCommunicationListener {
	...
	protected void reduceEnergy(double amount) {
		...
		if (comBus == null) {
			...
			/* Look below, we can see the setting up the instanteous energy level
			 * is done through update().
			 */
		}
	...
	public void update(NetworkInterface iface, ModuleCommunicationBus comBus) {
		...
		if (this.comBus == null) {
			this.comBus = comBus; // use addProperty() since it's the 1st time for Energylevel is stored into ComBus
			this.comBus.addProperty(ENERGY_VALUE_ID, this.currentEnergy);
			this.comBus.subscribe(ENERGY_VALUE_ID, this);
			
			// Extra method added by Yatu for tram nodes.
...
//		System.out.println("Bang Bang Bang, not for Trams");
		...
		// A new method created by Yatu 
//		System.out.println("Energy Level" + initEnergy[0]);
//		System.out.println("All of Settings From Settings.java :" + Settings.getterForprops());
		
		// the scanning is not equally time spaced.
		if (iface.isScanning()) {
			/* scanning at this update round */
			if (iface.getTransmitRange() > 0) {
				if (delta < 1) {
					reduceEnergy(this.scanEnergy * delta); // to finish one scanning, the time taken is 1s
				} else {
